...
public class OccupationRepository : Repository<Occupation, IdentityDbContext>, IOccupationRepository
{
    public OccupationRepository(IdentityDbContext dbContext) : base(dbContext)
    {
    }
    ...
    public async Task<bool> HasOccupationExists(string name, int? id, CancellationToken cancellationToken  )
    {

        return await Queryable().AnyAsync(x => x.Name == name && x.Id != id && !x.IsDeleted);
    }

    public async Task<SearchListModel<Occupation>> SearchAsync(SearchQueryModel<SearchOccupationsQueryFilterModel> searchQuery, CancellationToken cancellationToken)
    {
