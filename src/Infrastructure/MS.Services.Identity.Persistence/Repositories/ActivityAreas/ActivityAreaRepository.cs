...
public class ActivityAreaRepository : Repository<ActivityArea, IdentityDbContext>, IActivityAreaRepository
{
    public ActivityAreaRepository(IdentityDbContext dbContext) : base(dbContext)
    {
    }
   ...
   => await Queryable().Where(x => !x.IsDeleted && x.Status != Domain.Enums.StatusEnum.Inactive)
    ...
    public async Task<bool> HasActivityAreaExists(string name, int? id, CancellationToken cancellationToken  )
    {
        return await Queryable().AnyAsync(x => x.Name == name && x.Id != id && !x.IsDeleted);
    }

    public async Task<SearchListModel<ActivityArea>> SearchAsync(SearchQueryModel<SearchActivityAreasQueryFilterModel> searchQuery, CancellationToken cancellationToken)
    {
