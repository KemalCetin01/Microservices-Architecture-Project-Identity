...
public class SectorRepository : Repository<Sector, IdentityDbContext>, ISectorRepository
{
    public SectorRepository(IdentityDbContext dbContext) : base(dbContext)
    {
    }
    ...
    => await Queryable().Where(x => !x.IsDeleted && x.Status != Domain.Enums.StatusEnum.Inactive)
        ...
        .ToListAsync();
    public async Task<bool> HasSectorExists(string name, int? id, CancellationToken cancellationToken  )
    {

        return await Queryable().AnyAsync(x => x.Name == name && x.Id != id && !x.IsDeleted);
    }

    public async Task<SearchListModel<Sector>> SearchAsync(SearchQueryModel<SearchSectorsQueryFilterModel> searchQuery, CancellationToken cancellationToken)
    {
